...
import it.uniroma3.siw.spring.controller.validator.RottaValidator;
...
public class RottaController {
	...
	public String aggiungiRottaPOST (@Valid @ModelAttribute("rotta") Rotta rotta,
		...
		rotta.setUser(driver);
		rotta.setFurgone(furgone);
		
		rottaValidator.validate(rotta , bindingResult);

		if (rotta.getId() != null ) rottaDaSalvare = rottaService.findById(rotta.getId());

		if(!bindingResult.hasErrors()) {
			rottaDaSalvare.setData(rotta.getData());
			rottaDaSalvare.setCodiceRotta(rotta.getCodiceRotta());
